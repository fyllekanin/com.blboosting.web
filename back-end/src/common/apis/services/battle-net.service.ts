...
export class BattleNetService {
    ...
    static async getOath(region: BattleNetRegions, code: string, scope: string): Promise<BattleNetOauth2> {
        ...
        return await axios.post(this.AUTH_URL.replace('{{REGION}}', region), formData, {
            headers: {
    ...
    static async getWoWProfile(region: BattleNetRegions, accessToken: string): Promise<BattleNetProfile> {
        return await axios.get(`${this.BASE_URL.replace('{{REGION}}', region)}/profile/user/wow`, {
            headers: {
            ...
            .catch(err => {
                console.error(`WoW profile error!`);
                console.error(`Arguments: ${region}, ${accessToken}`);
    ...
    static async getCharacterAssets(region: BattleNetRegions, slug: string, character: string, accessToken: string): Promise<{
        ...
        return await axios.get(`${this.BASE_URL.replace('{{REGION}}', region)}/profile/wow/character/${slug}/${characterName}/character-media`, {
            headers: {
                Accept: 'application/json',
                'Content-Type': 'application/json',
                Authorization: `Bearer ${accessToken}`
            ...
            .catch(err => {
                console.error(`Character asset error!`);
                console.error(`Arguments: ${region}, ${slug}, ${character}`);
                console.error(`Error: ${err}`);
                return null;
            });
    }
    ...
    private static async getData<T>(region: BattleNetRegions, path: string, queryParameters: string): Promise<T> {
        ...
        const url = `${this.BASE_URL.replace('{{REGION}}', region)}${path}?${queryParameters}`;
    ...
    private static async getApplicationOauth(region: BattleNetRegions): Promise<BattleNetOauth2> {
        ...
        return await axios.post(this.AUTH_URL.replace('{{REGION}}', region), params, {
            headers: {
                'Authorization': 'Basic ' + Buffer.from(`${process.env.BATTLE_NET_CLIENT_ID}:${process.env.BATTLE_NET_CLIENT_SECRET}`)
                    .toString('base64')
            }
